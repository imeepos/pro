...
import { GraphQLDataAdapter } from './graphql-data.adapter';
import {
  RawData,
  RawDataFilters,
  RawDataListResponse,
...
export class DataService {
  ...
  constructor(private graphqlAdapter: GraphQLDataAdapter) {}

  loadDataList(filters?: RawDataFilters): void {
    this.setLoading(true);
    this.clearError();

    this.graphqlAdapter.findAll(filters)
      .pipe(
  ...
  createData(dto: CreateRawDataSourceDto): Observable<RawData> {
    ...
    // Note: GraphQL mutations for CRUD operations would need to be implemented
    // For now, return a placeholder that triggers data refresh
    console.warn('createData: GraphQL mutations not yet implemented');
    this.setLoading(false);
    this.loadDataList();
    this.loadStats();
  ...
  updateData(id: string, updates: UpdateRawDataDto): Observable<RawData> {
    ...
    // Note: GraphQL mutations for CRUD operations would need to be implemented
    console.warn('updateData: GraphQL mutations not yet implemented');
    this.setLoading(false);
    return of({} as RawData);
  }
  ...
  deleteData(id: string): Observable<void> {
    ...
    // Note: GraphQL mutations for CRUD operations would need to be implemented
    console.warn('deleteData: GraphQL mutations not yet implemented');
    this.setLoading(false);
  ...
  processData(id: string): Observable<RawData> {
    ...
    console.warn('processData: GraphQL mutations not yet implemented');
    this.setLoading(false);
    return of({} as RawData);
  }
  ...
  reprocessData(id: string): Observable<RawData> {
    ...
    console.warn('reprocessData: GraphQL mutations not yet implemented');
    this.setLoading(false);
    return of({} as RawData);
  }
  ...
  searchData(query: string): Observable<RawData[]> {
    return this.graphqlAdapter.search(query, 1, 50).pipe(
      catchError(() => of([]))
    );
  }

  getDataByStatus(status: ProcessingStatus): Observable<RawData[]> {
    return this.graphqlAdapter.findByStatus(status, 100).pipe(
      catchError(() => of([]))
    );
  }
